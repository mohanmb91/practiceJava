	...
	public class LongestCommonSubsequence {
	        ...
	        public static void main(String[] args) {
		        ...
		        for(int column = second.length() -1;column >= 0;){
		        	...
		        	if((row > 0	) && (column < secondLength+1 && column >= 0) && c[row][column] == c[row - 1][column]){
		        		row = row-1;
		        	}else if((row >= 0) && (column < secondLength+1 && column >= 0) && c[row][column] == c[row][column-1]){
		        		column = column -1;
		        	}else{
		        		if((column+1 < secondLength+1 && column+1 >= 0) && (column < secondLength+1 && column >= 0)){
